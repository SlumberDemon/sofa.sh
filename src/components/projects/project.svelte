...
>
  ...
  <div>
    <div class="flex justify-between items-center">
      ...
      <div class="text-neutral-500 dark:text-neutral-400 cursor-not-allowed">
        {state}
      </div>
    </div>

    <div
      class="lg:text-9xl lg:my-4 md:my-4 sm:my-4 my-8 dark:text-white md:text-8xl sm:text-7xl text-5xl text-ellipsis truncate justify-center flex lg:block md:block sm:block pb-4"
    >
      {key}
    </div>

    <div
      class="flex justify-between items-center select-none dark:text-white lg:flex-row md:flex-row sm:flex-row flex-col"
    >
      <div class="flex gap-2 flex-row items-center overflow-x-auto max-w-full">
        {#each stack as s}
          <div class="border border-neutral-200 rounded-full p-1 px-4 text-sm">
            {s}
          </div>
        {/each}
      </div>
      {#if !expandedSwitch}
        <div
          class="italic font-thin text-sm lg:mt-0 md:mt-0 sm:mt-0 mt-2"
    ...
    {#if expandedSwitch}
      ...
      >
        <hr class="border-t border-neutral-200 dark:border-neutral-800 my-6" />
        <div
        ...
        <div class="flex flex-row lg:gap-6 md:gap-6 sm:gap-4 gap-2 mt-2">
          {#if links}
            {#each links as link}
              <a
                href={link.url}
        ...
        <div
          class="italic font-thin text-sm flex justify-end select-none mt-2"
          in:blur|global={{ duration: 2400 }}
          out:fly|global
        >
          Click to Close
